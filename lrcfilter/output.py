...
from lrcfilter.config import INCLUDE_TIMESTAMP, OUTPUT_ENCODING
...
def _write_instrumental_file(
    ...
    with open(output_path, "w", encoding=OUTPUT_ENCODING) as f:
        ...
        for audio_file, result in tracks:
            f.write(f"{audio_file.path}\n")
            f.write(
                f"  # Words detected: {result.word_count}, Speech duration: {result.speech_duration:.1f}s\n"
            )
            f.write(f"  # Confidence: {result.confidence:.1%}\n")
            f.write("\n")
...
def _write_mismatch_file(
    ...
    """
    filename = (
        f"{filename_prefix}_metadata_mismatches.txt"
        if filename_prefix
        else "metadata_mismatches.txt"
    )
    output_path = output_dir / filename
